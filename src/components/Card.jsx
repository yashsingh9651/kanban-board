...
export const Card = ({card}) => {
  ...
  return (
    <>
    <div className='bg-gray-200 w-full p-2 md:min-h-[400px] md:max-h-[550px] overflow-y-auto custom'>
        ...
        {/* Add new task... */}
        {!showtask?<div className='w-full bg-white rounded p-2 mt-2 shadow-[1px_1px_0_0_rgba(0,0,0,0.2)] hover:bg-gray-100' onClick={()=>{setShowtask(!showtask)}}>Add Task</div>
        :
        <div className='w-full bg-white rounded p-2 mt-2 shadow-[1px_1px_0_0_rgba(0,0,0,0.2)] hover:bg-gray-100'>
          <div><textarea value={newTaskTitle} onChange={(e)=>{setNewTaskTitle(e.target.value)}} name="task" className='w-full rounded border border-gray-400 p-1' style={{resize:'none'}} autoFocus></textarea></div>
